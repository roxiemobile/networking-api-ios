...
open class InMemoryCookieStorage: HTTPCookieStorage, AbstractClass {
...
//    private let syncQ = DispatchQueue(label: "org.swift.HTTPCookieStorage.syncQ")
    ...
    private let syncQ = DispatchQueue(label: "com.roxiemobile.InMemoryCookieStorage.syncQ")
...
//    }
    ...
    open override class var shared: HTTPCookieStorage {
        Roxie.fatalError("Not supported.")
    }
...
//    }
    ...
    open override class func sharedCookieStorage(forGroupContainerIdentifier identifier: String) -> HTTPCookieStorage {
        Roxie.fatalError("Not supported.")
    }
...
extension InMemoryCookieStorage {
    ...
    open override func getCookiesFor(_ task: URLSessionTask, completionHandler: @escaping ([HTTPCookie]?) -> Void) {
        DispatchQueue.global(qos: .utility).async {
            ...
            if let link = task.currentRequest?.url {
                cookies = self.cookies(for: link)
            }
...
// ERROR: Method 'cookies(for:)' with Objective-C selector 'getCookiesForTask:completionHandler:' conflicts
//        with method 'getCookiesFor(_:completionHandler:)' with the same Objective-C selector

//    @available(iOS 8.0, *)
//    open override func cookies(for task: URLSessionTask) async -> [HTTPCookie]? {
//
//        guard let link = task.currentRequest?.url else {
//            return nil
//        }
//
//        return cookies(for: link)
//    }
}
...
// MARK: - Construction
    ...
    public convenience init(cookies: [HTTPCookie]) {
        self.init()
        setCookies(cookies)
    }
...
//    }
    ...
    fileprivate func validFor(host: String) -> Bool {
...
// HTTPCookie.swift
// @link https://github.com/apple/swift-corelibs-foundation/blob/main/Sources/FoundationNetworking/HTTPCookie.swift

extension HTTPCookiePropertyKey {

    // For Cocoa compatibility
    fileprivate static let created = HTTPCookiePropertyKey(rawValue: "Created")
}
